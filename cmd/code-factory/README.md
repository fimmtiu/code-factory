...
- Set `terminal` in `.code-factory/settings.json` to `"iterm2"`, `"terminal"`, `"cmux"`, or `"orca"` (default: `"iterm2"`).
  - Orca only resolves worktrees it manages, so import your repository into Orca first.
...
| `terminal` | `"iterm2"` | Terminal emulator for opening worktrees and notifications (`"iterm2"`, `"terminal"`, `"cmux"`, or `"orca"`) |
