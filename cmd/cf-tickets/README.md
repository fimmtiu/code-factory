...
Updates a ticket's phase (e.g. `implement`, `refactor`, `review`, `done`) and optionally its status (defaults to `idle`). Setting phase to `done` rebases the ticket's branch onto its parent project's worktree (or the repo's default branch) and fast-forwards the parent to the rebased tip, then removes the worktree. If the rebase hits conflicts, it is left in progress in the ticket's worktree for manual resolution.
